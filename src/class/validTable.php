...
class validTable {
  ...
  //Checking values ​​from quarters and year for tables
  function validationQuarters(array $table) {
    $quarter[] = $this->quarterValue(floatval($table['jan'] ?? 0), floatval($table['feb'] ?? 0), floatval($table['mar'] ?? 0));
    $quarter[] = $this->quarterValue(floatval($table['apr'] ?? 0), floatval($table['may'] ?? 0), floatval($table['jun'] ?? 0));
    $quarter[] = $this->quarterValue(floatval($table['jul'] ?? 0), floatval($table['aug'] ?? 0), floatval($table['sep'] ?? 0));
    $quarter[] = $this->quarterValue(floatval($table['oct'] ?? 0), floatval($table['nov'] ?? 0), floatval($table['dec'] ?? 0));
  ...
  //    Checks the value of one table.
  //    Checks for gaps between months, quarters, and years.
  function validationTable($table, &$startPos = [], &$endPos = []) {
    $start = FALSE;
    $end = FALSE;
    $old_year = NULL;
    $prevYear = NULL;
  ...
  //    Checks values between multiple tables
  function validationTables($table) {
    $tables = $table['arr'];
    $count = count($tables);
    $year_arr = [];
    ...
    for ($current_year = $min_year; $current_year <= $max_year; $current_year++) {
      ...
      if ($count_year > 1) {
        if ($this->array_has_empty_item($year_month_data)) {
          return FALSE;
        }
        else {
          continue;
  ...
  //   Checks the values for the fill between several tables
  function array_has_empty_item($array) {
    foreach ($array as $element) {
      foreach ($element as $man_value) {
        if (array_filter($element, function ($name) {
          return $name === '0' ? TRUE : !empty($name);
        })) {
          if ($man_value === "") {
            return TRUE;
          }
